...
│   ├── TC.md                    (Test Cases - 70+ validation assertions)
│   └── paper.pdf                (Reference paper)
├── src/
│   ├── config.py                (Centralized configuration & hyperparameters)
│   ├── make_data.py             (CIFAR-10 → PNG conversion)
│   ├── rotation.py              (Pretext task: 4-way rotation prediction)
│   ├── make_batches.py          (Group creation from rotation loss)
│   ├── main.py                  (DTS-based Active Learning loop)
│   ├── loader.py                (Data loaders for training)
│   └── models/
│       ├── __init__.py
│       └── resnet.py            (ResNet18 architecture)
├── checkpoint/                  (Training checkpoints - generated)
├── loss/                        (Loss logs and batch groupings - generated)
├── DATA/                        (Image data - generated)
│   ├── train/
│   └── test/
└── (output files)
    ├── performance_list.txt     (Final accuracy metrics)
    └── main_epoch_end.txt       (Per-cycle accuracy log)
```

**Key Directories:**
- `src/` - All Python source code (organized by function)
- `docs/` - Comprehensive documentation (SDD, TC, paper)
- `checkpoint/` - Trained model weights (generated at runtime)
- `loss/` - Pretext task loss logs and group assignments (generated at runtime)
- `DATA/` - Image dataset in PNG format (generated by `make_data.py`)
